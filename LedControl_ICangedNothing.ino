...
void setup() {
  ...
  BLE.scanForUuid("afe10a6b-72e3-4691-855b-dcccdfd0489a");
}
...
void controlLed(BLEDevice peripheral) {
  ...
  BLECharacteristic buttonLED = peripheral.characteristic("afe10a6b-72e3-4691-855b-dcccdfd0489a");

  if (!buttonLED) {
    Serial.println("Peripheral does not have LED characteristic!");
    peripheral.disconnect();
    return;
  } else if (!buttonLED.canWrite()) {
  ...
  while (peripheral.connected()) {
    ...
    if (oldButtonState != buttonState) {
      ...
      if (buttonState) {
        ...
        buttonLED.writeValue((byte)0x01);
      } else {
        Serial.println("button released");

        // button is released, write 0x00 to turn the LED off
        buttonLED.writeValue((byte)0x00);
      }
    }
  }
